"""
================================================================
 SISTEMA DE OPTIMIZACIÓN DE ELECTROLINERAS - BUCARAMANGA
 Proyecto Integrador - Entrega Final 2026-1
 Asignaturas: Algoritmos y Programación – Matemáticas Discretas
 Profesores: Cesar Augusto Aceros Moreno, PhD
              Rosaura Gutiérrez Almeyda, MSc
----------------------------------------------------------------
 Integrantes:
   • Camilo Amaya Mora
   • Johen Gustavo Castañeda García
   • Hector Felipe Higuera
 Programa: Ingeniería en Ciencia de Datos – UIS | Semestre 2026-1
================================================================

 NOTA TÉCNICA DEL MODELO:
   Se implementa un DÍGRAFO PONDERADO G = (V, E, W) usando
   nx.DiGraph(), lo que significa que la ruta UIS→Homecenter
   puede tener un peso distinto a Homecenter→UIS, reflejando
   calles de un solo sentido y rutas alternativas reales de
   Bucaramanga.

   Factores adicionales incorporados:
     • Tráfico por hora del día (hora pico AM/PM, madrugada)
     • Incidentes viales aleatorios (accidentes, obras, cierres)
     • Condiciones climáticas (lluvia, calor, tormenta)
     • Modelo de IA supervisado: Random Forest para predecir
       la electrolinera con mayor demanda según condiciones
================================================================
"""
...
# ── Importaciones de ML (opcionales) ───────────────────────────────
# Se usa "from __future__ import annotations" para evaluación diferida,
# y TYPE_CHECKING para declarar tipos sin importar en runtime.
# El truco definitivo para Pylance: declarar TODO antes del try/except
# con tipo Any, luego sobreescribir en el try. Así Pylance nunca ve None.
from typing import TYPE_CHECKING, Any

# Pre-declaración con tipo Any → Pylance nunca marca "possibly unbound"
# ni busca atributos en None.  El guard ML_DISPONIBLE impide uso en runtime.
np: Any = None
...
try:
    ...
    # ⚠ BaseException captura ImportError Y KeyboardInterrupt.
    # KeyboardInterrupt ocurre con numpy < 2.1 en Python 3.13.
    # FIX: pip install --upgrade "numpy>=2.1" scikit-learn
    ML_DISPONIBLE = False
    # Las variables ya tienen valor Any (None) → el sistema sigue funcionando
    # en modo degradado.  Todas las funciones de IA tienen el guard:
    #   if not ML_DISPONIBLE: print("..."); return

# ── BLOQUE 2: DATOS FIJOS DEL SISTEMA ─────────────────────────
...
# Multiplicadores de peso efectivo por hora del día
# Reflejan la congestión real de Bucaramanga
TRAFICO_POR_HORA = {
...
# Tipos de incidentes viales con probabilidad y factor de impacto
INCIDENTES_POSIBLES = [
    {"tipo": "Accidente vial",   "factor": 2.20, "prob": 0.05},
    {"tipo": "Obra en vía",      "factor": 1.65, "prob": 0.08},
    {"tipo": "Manifestación",    "factor": 3.00, "prob": 0.02},
    {"tipo": "Corte de vía",     "factor": 9.99, "prob": 0.01},  # ruta bloqueada
    {"tipo": "Semáforo dañado",  "factor": 1.35, "prob": 0.07},
    {"tipo": "Vía inundada",     "factor": 5.00, "prob": 0.03},  # común en lluvia
]

# ── ARISTAS DIRIGIDAS (A→B ≠ B→A) ─────────────────────────────
# Formato: (origen, destino, km_base)
# Los pesos distintos por dirección reflejan:
#   - Calles de un solo sentido (Bucaramanga tiene muchas)
#   - Rutas alternativas obligatorias
#   - Diferencias topográficas (subir vs bajar lomas)
ARISTAS_DIRIGIDAS = [
...
historial_recargas: list[dict[str, Any]] = []   # estructura de datos homogénea
modelo_rf_clf:  Any = None       # RandomForestClassifier (Any evita errores Pylance)
modelo_rf_reg:  Any = None       # RandomForestRegressor
modelo_entrenado   = False


# ================================================================
# BLOQUE 3: UTILIDADES DE VALIDACIÓN
# (Rúbrica: valida negativos, fuera de rango y caracteres)
# ================================================================

def validar_entero_positivo(mensaje: str) -> int:
    """Acepta SOLO enteros estrictamente > 0. Rechaza letras, negativos y cero."""
    while True:
...
def validar_rango_entero(mensaje: str, minimo: int, maximo: int) -> int:
    """Acepta enteros en el rango [minimo, maximo]. Valida caracteres y rango."""
    while True:
        entrada = input(mensaje).strip()
        if not entrada:
...
def validar_flotante_rango(mensaje: str, minimo: float, maximo: float) -> float:
    """Acepta flotantes en [minimo, maximo]."""
    while True:
        entrada = input(mensaje).strip()
        if not entrada:
            print("   [!] No ingresó ningún valor.")
...
# ================================================================
# BLOQUE 4: SISTEMA DE TRÁFICO, CLIMA E INCIDENTES
# ================================================================

def obtener_factor_trafico(hora: int, clima_id: int) -> float:
    """
    Calcula el factor de tráfico efectivo combinando hora del día
    y condición climática. Factor > 1 significa más congestión.
    """
...
def simular_incidente_en_arista(u: int, v: int, clima_id: int) -> dict | None:
    """
    Evalúa aleatoriamente si ocurre un incidente en la arista (u→v).
    En lluvia o tormenta, la probabilidad de inundación aumenta.
    Retorna el incidente ocurrido o None si no hay incidente.
    """
    candidatos = list(INCIDENTES_POSIBLES)

    # En climas adversos, aumentar probabilidad de vía inundada
    if clima_id in (1, 3):
        candidatos = [
...
def calcular_peso_efectivo(peso_base: float, hora: int,
    ...
    """
    Peso efectivo de una arista considerando:
      peso_base × factor_trafico × (factor_incidente si aplica)
    El clima ya está embebido en el factor de tráfico.
    """
...
def actualizar_pesos_grafo(hora: int, clima_id: int,
    ...
    """
    Recorre todas las aristas del dígrafo y actualiza su peso efectivo
    según hora, clima e incidentes aleatorios.
    Retorna dict con los incidentes detectados para mostrar al usuario.
    """
...
# ================================================================
# BLOQUE 5: CONFIGURACIÓN DEL GRAFO DIRIGIDO
# ================================================================

def configurar_grafo() -> None:
    """
    Construye el dígrafo ponderado G = (V, E, W) del área
    metropolitana de Bucaramanga.

    V: 18 nodos (8 electrolineras + 10 puntos fijos)
    E: 90+ aristas DIRIGIDAS con pesos asimétricos
       A→B puede tener diferente peso que B→A

    Cada arista almacena 'peso_base' (km reales) y
    'weight' (peso efectivo con tráfico, actualizable).
    """
    ...
    # Verificar conectividad débil (existe camino entre todo par ignorando dirección)
...
# ================================================================
# BLOQUE 6: NÚCLEO DE RUTEO – DIJKSTRA SOBRE DÍGRAFO
# ================================================================

def dijkstra_electrolinera_cercana(nodo_origen: int) -> tuple:
    """
    Aplica Dijkstra sobre el dígrafo G para encontrar la
    electrolinera más cercana desde nodo_origen según los
    pesos efectivos actuales (con tráfico e incidentes).

    Retorna: (id_elec, distancia_km, lista_ruta)
...
def distancias_a_todas_electrolineras(nodo_origen: int) -> list:
    """
    Retorna lista de distancias Dijkstra desde nodo_origen a
    cada electrolinera (en orden 0-7). Usa float('inf') si no hay ruta.
    Útil como features para el modelo de ML.
    """
    distancias = []
...
# ================================================================
# BLOQUE 7: INFORMACIÓN DE VEHÍCULOS
# ================================================================
...
# ================================================================
# BLOQUE 8: MOTOR DE SIMULACIÓN
# (Dijkstra + tráfico + incidentes + clima + estadísticas)
# ================================================================

def simular_recorridos() -> None:
    """
    Simula n recorridos aleatorios en el área metropolitana.
    Por cada recorrido:
      1. Selección aleatoria de origen, destino, vehículo,
         hora del día y condición climática.
      2. Se actualizan los pesos del dígrafo con tráfico
         e incidentes de esa "fotografía" temporal.
      3. Se calcula la ruta con Dijkstra y se simula el
         consumo de batería segmento a segmento.
      4. Si la batería cae al rango crítico (10%-20%),
         se invoca Dijkstra para la electrolinera más cercana.
      5. Se registra todo en historial_recargas.
    """
    ...
    # Estructuras homogéneas de conteo
    conteo_elec = {eid: 0 for eid in ELECTROLINERAS}  # vector de conteo
    conteo_veh  = {1: 0, 2: 0}
    ...
    for i in range(1, n + 1):
        # ── Parámetros aleatorios del recorrido ───────────────
        ...
        try:
            ...
            continue  # Sin ruta disponible, saltar recorrido

        # ── Simular consumo segmento a segmento ───────────────
        bat_actual       = bat_kwh
        recarga          = False
        nodo_critico     = origen
        bat_critica_pct  = 0.0
        ...
        # ── Si hay estado crítico: Dijkstra a electrolinera ───
        if recarga:
            ...
            if i <= 15 or i % 20 == 0:   # mostrar muestra para no saturar consola
                print(f"  ── #{i:03d} | ✔ OK | "
...
# ================================================================
# BLOQUE 9: CALCULAR RUTA MANUAL CON CONDICIONES
# ================================================================

def calcular_ruta_manual() -> None:
    """
    El usuario elige origen, hora y clima. El sistema actualiza
    los pesos del dígrafo y aplica Dijkstra para encontrar la
    electrolinera más cercana con esas condiciones específicas.
    """
    if not grafo_configurado:
        print("\n  [!] Configure primero el grafo (Opción 1).")
        return
    ...
    # Actualizar pesos con condiciones actuales
    ...
    if incidentes:
        ...
        for k, v in list(incidentes.items())[:3]:
            print(f"     • Arista {k}: {v}")
    # Estimación de consumo con el vehículo seleccionado
    ...
    # Ranking completo de electrolineras
...
# ================================================================
# BLOQUE 10: MODELO DE IA — RANDOM FOREST
# ================================================================

def entrenar_modelo_ia() -> None:
    """
    Entrena dos modelos supervisados con el historial generado:

    1. RandomForestClassifier:
       Predice QUÉ electrolinera usará el vehículo
       dado su estado (origen, batería, hora, clima, tráfico).

    2. RandomForestRegressor:
       Predice la DISTANCIA a recorrer hasta la electrolinera.

    Features:
       origen_id, tipo_vehiculo_id, bateria_inicio_pct,
       hora, es_hora_pico, clima_id, factor_trafico,
       dist_elec_0 … dist_elec_7

    Solo entrena con recorridos donde hubo recarga efectiva.
    """
    ...
    # Filtrar solo recorridos con recarga
...
def predecir_electrolinera() -> None:
    """
    Usa el modelo entrenado para predecir qué electrolinera usará
    un vehículo dado su estado actual, sin ejecutar Dijkstra.
    """
    ...
    # Actualizar pesos para esta situación
...
# ================================================================
# BLOQUE 11: GESTIÓN DE ARCHIVOS (.csv y .json)
# (Rúbrica: generan, leen y guardan datos en archivos)
# ================================================================

def exportar_datos() -> None:
    """Exporta el historial completo a archivos .csv y .json."""
...
def leer_archivos() -> None:
    """Lee y muestra archivos .csv o .json previamente generados."""
    archivos = sorted([
...
# ================================================================
# BLOQUE 12: ESTADÍSTICAS Y REPORTES
# ================================================================

def ver_estadisticas() -> None:
    """Muestra estadísticas globales del historial acumulado."""
    if not historial_recargas:
    ...
    # Distancia promedio hasta electrolinera
    if recargas:
...
# ================================================================
# BLOQUE 10-B: ENTRENAMIENTO DESDE CSV ACUMULADO
# (Rúbrica: mejora el accuracy usando dataset histórico completo)
# ================================================================

def entrenar_desde_csv() -> None:
    """
    Carga un archivo CSV exportado previamente y entrena el modelo
    con TODOS los registros históricos acumulados. Esto mejora
    significativamente la accuracy respecto a entrenar solo con
    los datos de la sesión actual.

    Para llegar a accuracy 80%+ se recomiendan 3000-5000 registros.
    Con 185+ registros se obtiene 45-65%.
    """
    global modelo_rf_clf, modelo_rf_reg, modelo_entrenado
...
# ================================================================
# BLOQUE 13-A: ANÁLISIS ACADÉMICO DE TEORÍA DE GRAFOS
# Métricas de centralidad + benchmark Dijkstra vs Floyd-Warshall
# (Requerimiento: Matemáticas Discretas / rúbrica 20%)
# ================================================================

def analizar_centralidad_red() -> None:
    """
    Calcula tres métricas de centralidad sobre el dígrafo G:

    1. Betweenness Centrality — nodo que más rutas óptimas atraviesa.
       bc(v) = Σ σ(s,t|v) / σ(s,t)   ∀s,t ∈ V
       Indica cuál electrolinera o punto fijo es "cuello de botella".

    2. Closeness Centrality — electrolinera más accesible desde toda la red.
       cc(v) = (n-1) / Σ d(v,u)

    3. Grado de entrada ponderado — demanda potencial de recarga.
       Suma de pesos de aristas entrantes a cada electrolinera.
    """
    if not grafo_configurado:
        print("\n  [!] Configure primero el grafo (Opción 1).")
        return
...
def benchmark_dijkstra_vs_floyd() -> None:
    """
    Compara el costo computacional real de:
      - Dijkstra (todos los pares)  → O(V·(E+V)·log V)
      - Floyd-Warshall              → O(V³)

    Con V=18 y E=90, Floyd-Warshall es competitivo en grafos densos
    pero Dijkstra escala mejor a redes viales de miles de nodos.
    """
    if not grafo_configurado:
        print("\n  [!] Configure primero el grafo (Opción 1).")
        return
...
# ================================================================
# BLOQUE 13: VISUALIZADOR INTERACTIVO DEL GRAFO  (D3.js / HTML)
# Abre visualizador_electrolineras.html en el navegador.
# Ese archivo contiene el dígrafo completo con D3.js v7:
#   • Grafo fuerza-dirigida con posiciones geográficas fijas
#   • Animación Dijkstra paso a paso
#   • Tráfico horario en tiempo real (slider)
#   • Selector de clima (factores de congestión)
#   • Áreas de Voronoi de cobertura por electrolinera
#   • Simulación de vehículo animado
#   • Métricas de betweenness centrality
# ================================================================
...
def visualizar_grafo() -> None:
    """
    Abre el visualizador D3.js interactivo del dígrafo ponderado.
    El archivo 'visualizador_electrolineras.html' debe estar en el
    mismo directorio que este script.

    Características del visualizador:
      - Grafo dirigido con posiciones geográficas reales de Bucaramanga
      - Animación paso a paso del algoritmo Dijkstra
      - Control de hora del día (factor de tráfico horario)
      - Selector de clima (Despejado / Lluvia / Calor / Tormenta)
      - Overlay de Voronoi (zonas de cobertura por electrolinera)
      - Simulación de vehículo eléctrico recorriendo la ruta
      - Betweenness centrality visible en el tamaño de los nodos
    """
    import webbrowser

    # Buscar el HTML en el directorio del script y en el directorio actual
    candidatos = [
...
# ================================================================
# BLOQUE 14: MENÚ PRINCIPAL
# (Ciclo while por centinela — rúbrica: opción 9 = salir)
# ================================================================
...
def main() -> None:
    ...
    while opcion != 9:   # centinela = 9
        mostrar_menu()
        entrada = input("  Seleccione una opción [1-9]: ").strip()

        # Opción especial 5b — entrenar desde CSV
        if entrada.lower() == '5b':
            entrenar_desde_csv()
            opcion = 0
            continue
